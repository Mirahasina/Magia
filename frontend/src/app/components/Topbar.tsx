...
export function Topbar({ user, onLogout, onToggleSidebar, isSidebarOpen }: TopbarProps) {
    ...
    return (
        <header className="h-16 border-b border-gray-200/30 bg-white/40 backdrop-blur-xl flex items-center justify-between px-4 sm:px-8 sticky top-0 z-30">
            ...
            <div className="flex items-center gap-8">
                <button className="relative p-2 text-blue-900 bg-blue-50/50 rounded-full hover:bg-blue-100 transition-colors duration-300">
                    <Bell className="w-5 h-5" />
                    <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-blue-900 rounded-full border-2 border-white shadow-sm"></span>
                </button>
