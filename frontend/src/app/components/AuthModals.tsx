...
import { Play, X, Eye, EyeOff, Loader2, Check } from "lucide-react";
...
export function AuthModals({ isOpen, onClose, defaultView = "signup", onSuccess, initialEmail }: AuthModalProps) {
    ...
    }, [isOpen, defaultView, initialEmail]);
    ...
    // Update onSuccess calls to check for invitation
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[900px] p-0 overflow-hidden border-none shadow-2xl rounded-2xl animate-in fade-in zoom-in-95 duration-300 bg-white">
                <button
                    onClick={onClose}
                ...
                <div className="flex flex-col md:flex-row h-full min-h-[580px] relative">

                    <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center relative bg-white z-10">
                        <div className="flex items-center gap-2 mb-6">
                            <div className="w-8 h-8 rounded-none flex items-center justify-center p-1">
                                <Logo />
                            </div>
                        </div>
                        ...
                        >
                            {errorMsg && (
                                <div className="p-3 bg-red-50 border border-red-100 text-red-600 text-[11px] rounded-xl font-bold animate-in fade-in slide-in-from-top-1">
                                    {errorMsg}
                                </div>
                            )}
                            {successMsg && (
                                <div className="p-3 bg-emerald-50 border border-emerald-100 text-emerald-600 text-[11px] rounded-xl font-bold animate-in fade-in slide-in-from-top-1">
                                    {successMsg}
                                </div>
                            )}
