        ...
        <div className="h-full flex flex-col space-y-8 animate-page-fade overflow-hidden">
            <div className="space-y-6">
                <div className="flex items-start justify-between">
                    <div className="space-y-1">
                        <h1 className="magia-h1">Mon espace de travail</h1>
                        <div className="flex items-center gap-2 mt-1">
                            <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">
                                {usage.members} / {limits.max_members === null ? '∞' : limits.max_members} membres
                            </span>
                            <span className={cn("px-2 py-0.5 text-[8px] font-black uppercase tracking-widest rounded-full", planColor)}>
                                {plan}
                            </span>
                        </div>
                    </div>
                    <div className="flex flex-col items-end gap-1">
                        <button
                            ...
                            className={cn(
                                "px-5 py-2.5 rounded-lg text-[11px] font-black uppercase tracking-[0.2em] shadow-lg transition-all flex items-center gap-2",
                                canInviteMembers
                ...
                {showInviteForm && (
                    <div className="p-6 bg-white border border-gray-100 rounded-3xl shadow-xl space-y-4 animate-in slide-in-from-top-4 duration-300 flex-shrink-0 border-blue-100">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-900">Nouvelle invitation</h3>
                        <div className="flex gap-4">
                            <input
                                ...
                                className="flex-grow px-5 py-3 border border-gray-100 rounded-xl text-sm font-medium focus:outline-none focus:border-blue-900 bg-gray-50/50 transition-all uppercase placeholder:text-gray-300"
                            />
                            <select
                                value={inviteRole}
                                onChange={e => setInviteRole(e.target.value as 'viewer' | 'editor')}
                                className="px-5 py-3 border border-gray-100 rounded-xl text-[10px] font-black uppercase tracking-widest bg-gray-50/50 focus:outline-none cursor-pointer hover:border-blue-900 transition-all"
                            >
                        ...
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setShowInviteForm(false)} className="px-5 py-2.5 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-gray-700 transition-colors">Annuler</button>
                            <button
                                onClick={handleInvite}
                                disabled={inviting || !inviteEmail.trim()}
                                className="px-8 py-2.5 bg-blue-950 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-black transition-all"
                            >
                ...
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-12 space-y-4">
                    {loading ? (
                        <div className="py-8 text-center text-[10px] font-black text-gray-300 uppercase tracking-widest">Chargement...</div>
                    ) : members.length === 0 ? (
                        <div className="py-20 text-center border-2 border-dashed border-gray-100 rounded-3xl">
                            <p className="text-[11px] font-black uppercase tracking-widest text-gray-300">Aucun membre détecté dans l'espace</p>
                        </div>
                    ) : members.map((member) => (
                        <div key={member.id} className="flex items-center justify-between p-5 bg-white border border-gray-100 rounded-2xl hover:border-blue-200 hover:shadow-xl hover:-translate-y-0.5 transition-all shadow-sm group">
                            <div className="flex items-center gap-5">
                                <div className="w-12 h-12 bg-blue-900 rounded-xl flex items-center justify-center font-black text-white text-lg shadow-lg">
                                    {member.name.charAt(0).toUpperCase()}
                                </div>
                                <div>
                                    <div className="text-[12px] font-black text-gray-900 uppercase tracking-tight">{member.name}</div>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="flex flex-col items-end">
                                    <span className={cn(
                                        "px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full mb-1",
            ...
            {memberships.length > 0 && (
                <div className="space-y-12 pt-12 border-t border-gray-100">
                    <div className="space-y-1">
                        <h2 className="magia-h1 opacity-50">Workspaces rejoints</h2>
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">Workspaces dont vous êtes membre</p>
                    </div>
                        ...
                        {memberships.map((m) => {
                            ...
                            return (
                                <div key={m.id} className="space-y-6">
                                    <div className="p-6 bg-slate-50 border border-gray-100 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center group hover:border-blue-200 hover:bg-white hover:shadow-xl transition-all gap-4">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-black text-blue-900 uppercase tracking-[0.2em]">{m.role === 'editor' ? 'Droit Éditeur' : 'Lecture Seule'}</p>
                                            <h3 className="text-lg font-black text-gray-900 leading-tight uppercase tracking-tight">Workspace de {m.owner_name}</h3>
                                            <p className="text-[11px] text-gray-400 font-medium italic">{m.owner_email}</p>
                                        </div>
                                        <div className="flex items-center gap-4">
                                            <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none bg-white px-4 py-3 rounded-xl shadow-sm border border-gray-50">UNITÉ ACTIVE DEPUIS {new Date(m.joined_at).toLocaleDateString()}</span>
                                    ...
                                    {workspaceTeam && workspaceTeam.teammates.length > 0 && (
                                        <div className="ml-6 pl-6 border-l-2 border-slate-50 space-y-3">
                                            <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest pl-1">Mes Coéquipiers ({workspaceTeam.teammates.length})</h4>
                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                                {workspaceTeam.teammates.map((t, idx) => (
                                                    <div key={idx} className="flex items-center gap-3 p-3 bg-white border border-gray-50 rounded-xl hover:border-blue-100 hover:shadow-md transition-all">
                                                        <div className="w-9 h-9 bg-slate-100 rounded-lg flex items-center justify-center text-[10px] font-black text-slate-400 italic">
                                                            {t.name.charAt(0).toUpperCase()}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <div className="text-[10px] font-black text-slate-700 truncate uppercase">{t.name}</div>
                                                            <div className="text-[8px] font-bold text-slate-300 uppercase tracking-tighter">{t.role === 'editor' ? 'ÉDITEUR' : 'LECTEUR'}</div>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
